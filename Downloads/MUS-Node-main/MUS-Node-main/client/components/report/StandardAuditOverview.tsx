...
export const StandardAuditOverview: React.FC<Props> = ({ report }) => {
    ...
    const rows = useMemo(() => {
        ...
        const add = (items: { Parameters?: ScoredParameter[] } | undefined, category: string, icon: any, tab: string) => {
            if (items?.Parameters) {
                items.Parameters.forEach(p => {
                    if (p.Score > 0) {
                        all.push({
                            id: toId(p.ParameterName) + Math.random().toString(36).substr(2, 5), // Ensure uniqueness
                            category,
                            categoryIcon: icon,
                            tabName: tab,
        ...
        if (report['Strategy Audit expert']) {
            const strat = report['Strategy Audit expert'];
            add(strat.TrustSignalsAndCredibility, 'Strategy', BrainCircuit, 'Strategic Foundation');
            add(strat.TargetAudienceAlignment, 'Strategy', BrainCircuit, 'Strategic Foundation');
            add(strat.CompetitiveDifferentiation, 'Strategy', BrainCircuit, 'Strategic Foundation');
            add(strat.CallToActionStrategy, 'Strategy', BrainCircuit, 'Strategic Foundation');
        }
                                ...
                                <React.Fragment key={row.id}>
                                    ...
                                    >
                                        ...
                                        <td className="p-4 font-bold text-slate-900">
                                            {row.parameter.replace(/([A-Z])/g, ' $1').trim()}
                                        </td>
