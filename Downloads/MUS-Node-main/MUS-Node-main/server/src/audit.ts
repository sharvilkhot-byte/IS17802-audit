                ...
                try {
                    if (browserEndpoint) {
                        ...
                        browser = await puppeteer.launch({
                            headless: "new", // Try 'new' again with proper UA, it's actually more performant if UA is set
                            // pipe: true, // REMOVED: Reverting to WebSocket for broader compatibility
                            args: [
                                '--no-sandbox',
                                ...
                                '--disable-infobars',
                                '--no-zygote',
                                '--mute-audio', // Mute audio to prevent media stream crashes
                            ]
                        });
                    }
...
`;
                    return callApi(ai, systemInstruction, fullContent, schemas.competitorAuditSchema, primaryScreenshotBase64, screenshotMimeType, competitorScreenshotBase64);
                };
