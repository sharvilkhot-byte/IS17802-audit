...
export const getCompetitorSystemInstruction = () => `
  ...
  - **Categorized Comparisons (Face-offs)**:
    You MUST provide a comparison item for EVERY single parameter listed below. Do not skip any.
    ...
    4. **StrategyComparison**:
       - 'DomainClarity', 'PurposeClarity', 'TargetAudienceAlignment', 'TrustSignals', 'MarketPositioning', 'BrandAuthority'

    5. **AccessibilityComparison**:
  ...
  - **Opportunities**: You MUST provide exactly 3 actionable recommendations for the Primary site to beat the Competitor.
  
  **CRITICAL**: You must complete ALL sections, including the Strengths and Opportunities arrays. Do not leave them empty.
`;
...
export const getSchemas = () => {
    ...
    const competitorAuditSchema = {
        ...
        properties: {
            ExecutiveSummary: { type: Type.STRING },
            // Removed single ComparisonMatrix, added categorized comparisons
            UXComparison: {
                type: Type.ARRAY,
                items: {
                    type: Type.OBJECT,
        ...
        required: ['ExecutiveSummary', 'UXComparison', 'ProductComparison', 'VisualComparison', 'StrategyComparison', 'AccessibilityComparison', 'CompetitorStrengths', 'PrimaryStrengths', 'Opportunities']
    };


    return { uxAuditSchema, productAuditSchema, visualAuditSchema, strategyAuditSchema, accessibilityAuditSchema, criticalIssueSchema, competitorAuditSchema };
};
